/* test_base_sort.c */
#include "head.h"
#define MASS_TEST_TIME (100 * 100 * 100 * 100 * 10)
...
void get_lps(const char *, int *);
void getlps(const char *, int *); /* 声明getlps() 函数 */
void getlps(const char *s, int *lps){get_lps(s, lps);}
...
void test_sort(int n, char *name, void(*sort)(int [], int)) {
	int *v;
	int i;

	v = (int *) malloc(sizeof(int) * n);
	if (v == NULL) {
		printf("test_sort() error: malloc() return NULL\n");
		return;
	}
	for (i = 0; i < n; ++i)
		v[i] = rand() % 100;
	printf("testing: %-15s, Total: %4d... ", name, n);
	fflush(stdout);
	sort(v, n);
	if (is_sort_ok(v, n))
		printf("OK\n");
...
void test_all() {
	test_sort(200, "quick sort", qksort);
	test_sort(200, "insert sort", insort);
	test_sort(200, "bubble sort", bbsort);
	test_sort(200, "shell sort", shsort);
	test_sort(200, "radix sort", rdsort);
	test_sort(200, "heap sort", hpsort);
	test_sort(200, "merge sort", mgsort);
}
