...
dir()
    ...
    如果有实参，它会尝试返回该object的有效属性列表。
    如果object有一个名为 __dir__() 的方法，那么该方法将被调用，并且必须返回一个属性列表。
    默认的 dir() 机制对不同类型的object行为不同，它会试图返回最相关而不是最全的信息：
        如果object是模块对象，则列表包含模块的属性名称。
        如果object是类型或类对象，则列表包含它们的属性名称，并且递归查找所有基类的属性。
