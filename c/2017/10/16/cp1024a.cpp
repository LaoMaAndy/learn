/*
 * function: stringcmp, stringsch, stringvec, stringswap, stringsort
 */
#include "0head.cpp"
...
vector<string>::size_type stringvec(const string &s, vector<string> &v)
{
...
void vsqksort(vector<string>::iterator left, vector<string>::iterator right)
{
	...
	decltype(left) last = left;
	for (decltype(left) i = left + 1; i != right; ++i)
