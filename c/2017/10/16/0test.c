...
/* ------ String search ------- */
int searchstr(const char *word, const char *list[], int nlist)
{
	int low, high, mid;
	int r;
...
struct table *tbseek(struct table *tab[], int tsize, char *word)
{
	struct table *p;

	for (p = tab[tbhash(word, tsize)]; p != NULL; p = p->next)
		if (Strcmp(p->word, word) == 0)
			return p;
	return NULL;
}

struct table *tbupd(struct table *tab[], int tsize, char *word, char *def)
{
	unsigned hval;
	struct table *p;

	p = tbseek(tab, tsize, word);
	if (p == NULL)
	{
...
void tbprint(struct table *tab[], int tsize)
{
	int i;
	struct table *p;
