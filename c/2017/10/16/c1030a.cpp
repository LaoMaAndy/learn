/*
function: string, vs, bit, recursion
*/
#include "0head.cpp"
...
inline void bitset(unsigned &v, int n)
{
	v |= 1ul << n;
}
inline void bitclr(unsigned &v, int n)
{
	v &= ~(1ul << n);
}
inline bool bitchk(unsigned v, int n)
{
...
void viprn(const vector<int> &vi, unsigned i)
{
	if (i < vi.size())
	{
		cout << vi[i] << endl;
