...
#define BUF_S 100
void file_key(FILE *fp, const char *word[], int wsize)
{
	char buf[BUF_S];
	int i;
	int count[wsize];
...
struct tree *trseek(struct tree *root, char *word)
{
	int r;
	...
	else
	{
		...
		if (r < 0)
			root->left = trseek(root->left, word);
		else if (r > 0)
			root->right = trseek(root->right, word);
		else
			root->count++;
	}
	return root;
