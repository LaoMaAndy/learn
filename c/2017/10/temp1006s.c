...
char *Strrchr(const char *dest, int chr)
{
	...
	for (dest += Strlen(d) - 1; dest >= d; dest--)
		if (*dest == chr)
			return (char *) dest;
	return NULL;
}

int Strspn(const char *dest, const char *pref)
{
	const char *d = dest;
	
	for ( ; *dest != '\0'; dest++)
		if (Strchr(pref, *dest) == NULL)
			break;
	return dest - d;
}

int Strcspn(const char *dest, const char *pref)
{
	const char *d = dest;
...
char *Strpbrk(const char *dest, const char *pres)
{
	dest += Strcspn(dest, pres);
	if (*dest != '\0')
		return (char *) dest;
	return NULL;
}
