/*
function: stringcmp, stringsch, stringvct, vsswap, vsqksort, vssort
*/
#include "0head.cpp"
int stringcmp(const string &s, const string &t)
{
	decltype(string[0]) cs, ct;
	decltype(s.size()) ss = s.size(), ts = t.size(), i;
	for (i = 0; i != ss || i != ts; ++i)
	{
		cs = s[i];
		ct = t[i];
...
void vsswap(vector<string>::iterator left, vector<string>::iterator right)
{
	string temp;
	temp = *left;
	*left = *right;
