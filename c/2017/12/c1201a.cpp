...
size_t stringvx(const string &s, vector<string> &v, int (*fp)(int))
{
	size_t count = 0;
	bool inword = false;
	decltype(v.end()) t;
...
void vsuniq(vector<string> &src, vector<string> &dest)
{
	vector<string>::iterator t = src.begin();
	if (t != src.end())
		dest.push_back(*t++);
	while (t != src.end())
	{
		if (stringcmp(*(dest.end() - 1), *t) != 0)
			dest.push_back(*t);
		++t;
	}
}
