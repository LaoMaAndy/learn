/*
function: stringcmp, search, stringvs, vsswap, vssort, vsuniq
*/
#include "0head.cpp"
...
int stringcmp(const string &s, const string &t)
{
	...
	for (i = 0; i != ss && i != ts; ++i)
		if (s[i] != t[i])
			return s[i] - t[i];
	if (i == ss) return 0 - t[i];
	if (i == ts) return s[i];
}
vector<string>& stringvec(const string& s, vector<string>& v)
{
	bool inword = false;
	for (auto c : s)
		if (!isalnum(c))
...
{
	size_t i = 0, j = dest.size() - 1, ss = src.size();
	if (i != ss)
	{
