/*
function: stringcmp, stringsch, stringvec, vsswap, vssort, vsuniq
*/
#include "0head.cpp"
...
size_t stringsch(const string &x, vector<string> &v)
{
...
void vsquicksort(vector<string>::iterator left, vector<string>::iterator right)
{
	if (left >= right)
		return ;
	...
	for (auto i = left + 1; i < right; ++i)
		if (stringcmp(*i, *left) < 0)
			vsswap(++last, i);
	vsswap(left, last);
	vsquicksort(left, last);
	vsquicksort(last + 1, right);
}
void vssort(vector<string> &v)
{
	vsquicksort(v.begin(), v.end());
}
void vsuniq(vector<string> &dest, const vector<string> &src)
{
	size_t i = 0;
	auto ss = src.size();
	if (i != ss)
	...
	while (i != ss)
	{
		if (stringcmp(dest[dest.size() - 1], src[i]) != 0)
			dest.push_back(src[i]);
		++i;
	}
}
