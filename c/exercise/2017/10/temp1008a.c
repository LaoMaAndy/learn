/*
function: sort, search.
*/
#define NEED_FILE
#include "0head.c"

void swap(int v[], int i, int j)
{
	int temp;

	temp = v[i];
	v[i] = v[j];
	v[j] = temp;
}

void swapstr(char *v[], int i, int j)
{
	char *temp;

	temp = v[i];
	v[i] = v[j];
	v[j] = temp;
}

void quicksort(int v[], int left, int right)
{
	int i, last;

	if (left >= right)
		return;
	swap(v, left, (left + right) / 2);
	last = left;
	for (i = left + 1; i <= right; i++)
		if (v[i] < v[left])
			swap(v, ++last, i);
	swap(v, last, left);
	quicksort(v, left, last - 1);
	quicksort(v, last + 1, right);
}

void qksort(int v[], int n) 
{
	quicksort(v, 0, n - 1);
}

void quicksort2(int v[], int n)
{
	int i, last;

	if (n <= 1)
		return;
	swap(v, 0, rand() % n);
	last = 0;
	for (i = 1; i < n; i++)
		if (v[i] < v[0])
			swap(v, ++last, i);
	swap(v, 0, last);
	quicksort2(v, last);
	quicksort2(v + last + 1, n - last - 1);
}
#include <string.h>
int lstrcmp(const char *s, const char *t)
{
	return strcmp(s, t);
}

void quicksortstr(char *v[], int left, int right)
{
	int i, last;

	if (left >= right)
		return;
	swapstr(v, left, (left + right) / 2);
	last = left;
	for (i = left + 1; i <= right; i++)
		//if (Strcmp(v[i], v[left]) < 1)
		if (lstrcmp(v[i], v[left]) < 0)
			swapstr(v, ++last, i);
	swapstr(v, left, last);
	quicksortstr(v, left, last - 1);
	quicksortstr(v, last + 1, right);
}

void sortstr(char *v[], int n)
{
	quicksortstr(v, 0, n - 1);
}

void shsort(int v[], int n)
{
	int gap, i, j;
	
	for (gap = n / 2; gap > 0; gap /= 2)
		for (i = gap; i < n; i++)
			for (j = i - gap; j >= 0 && v[j] > v[j + gap]; j -= gap)
				swap(v, j, j + gap);
}

void bbsort(int v[], int n)
{
	int i, j;

	for (i = 0; i < n - 1; i++)
		for (j = 0; j < n - i - 1; j++)
			if (v[j] > v[j + 1])
				swap(v, j, j + 1);
}

int binsearch(int x, int v[], int n)
{
	int low, high, mid;
	int r;

	low = 0; 
	high = n - 1;
	while (low <= high)
	{
		mid = (low + high) / 2;
		r = v[mid] - x;
		if (r > 0)
			high = mid - 1;
		else if (r < 0)
			low = mid + 1;
		else
			return mid;
	}
	return -1;
}

int bin2(int x, int v[], int n)
{
	int low, high, mid;
	int r;

	low = 0;
	high = n;
	while (low < high)
	{
		mid = low + (high - low) / 2;
		r = v[mid] - x;
		if (r > 0)
			high = mid;
		else if (r < 0)
			low = mid + 1;
		else
			return mid;
	}
	return -1;
}

int searchstr(char *x, char *v[], int n)
{
	int low, high, mid;
	int r;

	low = 0;
	high = n;
	while (low < high)
	{
		mid = low + (high - low) / 2;
		r = Strcmp(v[mid], x);
		if (r > 0)
			high = mid;
		else if (r < 0)
			low = mid + 1;
		else
			return mid;
	}
	return -1;
}

/*
 * test these functions
 */

void test(void (*sort)(int [], int))
{
	int v[] = {12, 10, 8, 6, 4, 2, 0, 1, 3, 5, 7, 9, 11, 13};
	int n = sizeof(v) / sizeof(v[0]);
	int i;

	for (i = 0; i < n; i++)
		printf("%3d", v[i]);
	printf("\n");

	sort(v, n);

	for (i = 0; i < n; i++)
		printf("%3d", v[i]);
	printf("\n");
	printf("binsearch(0, v, n): %3d\n", binsearch(0, v, n));
	printf("binsearch(11, v, n): %3d\n", binsearch(11, v, n));
	printf("binsearch2(0, v, n): %3d\n", binsearch(0, v, n));
	printf("binsearch2(13, v, n): %3d\n", binsearch(13, v, n));
	printf("binsearch2(7, v, n): %3d\n", binsearch(7, v, n));
}


void test_list(int argc, char *argv[])
{
	FILE *fp;
	
	if (argc < 2)
	{
		printf("Usage: aaa + filename\n");
		return;
	}
	fp = fopen(argv[1], "r");
	if (fp == NULL)
	{
		printf("can't open %s.\n", argv[1]);
		return ;
	}

	char **list;
	int n;
	n = Getlist(fp, &list);
	if (n == 0)
	{
		printf("get 0 line from file.\n");
		return;
	}
	sortstr(list, n);
	for (int i = 0; i < n; i++)
		printf("%3d: %s\n", i, list[i]);
	int i = searchstr("int", list, n);
	if (i != -1)
		printf("search result: %d, %s\n", i, list[i]);
	fclose(fp);
}

/*
 * Main function
 */

int main(int argc, char *argv[])
{
	test_list(argc, argv);
	PRN_CAL(test(qksort));
	PRN_CAL(test(shsort));
	PRN_CAL(test(bbsort));
	PRN_CAL(test(quicksort2));

	return 0;
}





