...
int stkresize(stkhea *s, int n);
int stkpush(stkhead *s, stkdata d);
int stkpop(stkhead *s, stkdata *dp);
int stkgetop(stkhead *s, stkdata *dp);
int stkgetn(stkhead *s, stkdata *dp, int n);
...
int stkresize(stkhead *s, int n)
{
	...
	dp = (stkdata *) realloc(s->base, n * sizeof(stkdata));
	if (dp == 0)
		return 0;
	i = s->top - s->base;
	if (i > n)
...
int stkpop(stkhead *s, stkdata *d)
{
	if (s->top <= s->base)
		return 0;
	*d = *--s->top;
...
int stkgetop(stkhead *s, stkdata *d)
{
	if (s->top <= s->base)
		return 0;
	*d = *(s->top - 1);
...
int stkgetn(stkhead *s, stkdata *d, int n)
{
	if (s->top - n <= s->base)
		return 0;
	*d = *(s->top - n - 1);
