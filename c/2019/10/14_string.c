...
size_t Strspn(const char *dest, const char *spn) {
	const char *p = dest;

	for ( ; *dest != '\0'; ++dest)
		if (Strchr(spn, *dest) == NULL)
			break;
	return dest - p;
}
size_t Strcspn(const char *dest, const char *cspn) {
	const char *p = dest;

	for ( ; *dest != '\0'; ++dest)
		if (Strchr(cspn, *dest) != NULL)
			break;
	return dest - p;
}
char *Strpbrk(const char *dest, const char *cspn) {
	dest += Strcspn(dest, cspn);
