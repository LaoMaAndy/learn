...
void shsort(int v[], int n) {
	...
	for (gap = n / 2; gap > 0; gap /= 2)
		for (i = gap; i < n; ++i)
			for (j = i - gap; j >= 0 && v[j] > v[j + 1]; j -= gap)
				swap(v, j, j + gap);
}
...
void heapify(int v[], int n, int head) {
	...
	for (temp = v[head]; (child = 2 * head + 1) < n; head = child) {
...
void get_lps(const char *s, int lps[]) {
	int i, j, len;
...
int kmp_search(const char *dest, const char *src) {
	int dest_len, src_len, d, s, *lps;

	dest_len = strlen(dest);
	src_len = strlen(src);
	lps = (int *) malloc(sizeof(int) * src_len);
	if (lps == NULL)
		return -1;
	get_lps(src, lps);
	for (d = s = 0; d < dest_len && s < src_len; )
		if (dest[d] == src[s]) {
			++d;
			++s;
		}
