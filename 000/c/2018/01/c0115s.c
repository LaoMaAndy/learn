...
size_t Strcspn(const char *dest, const char *nopre);
char *Strpbrk(const char *dest, const char *nopre);
...
char *Squees(char *dest, char *sqz);
...
size_t Strcspn(const char *dest, const char *nopre)
{
	...
	for ( ; *dest != '\0'; ++dest)
		if (Strchr(nopre, *dest) != NULL)
			break;
	return dest - d;
}
char *Strpbrk(const char *dest, const char *nopre)
{
	dest += Strcspn(dest, nopre);
	if (*dest == '\0')
		return NULL;
	return (char *) dest;
}
...
{	
	...
	PRN_VAL(g, s);
	PRN_FUN(Strlen(f), lu);
	PRN_FUN(Strlen(Trim(f)), lu);
	PRN_VAL(f, s);
	PRN_FUN(Strlen(Trim(g)), lu);
	PRN_FUN(Strlen(""), lu);
